...
-module(hstreamdb_erlang_client).

-behaviour(supervisor).

-export([
    create_channel_pool/2,
    create_channel_pool/3,
    stop_channel_pool/1
]).
...
-spec create_channel_pool(term(), uri_string:uri_string()) ->
    supervisor:startchild_ret().
create_channel_pool(Name, URL) ->
    create_channel_pool(Name, URL, #{}).

-spec create_channel_pool(term(), uri_string:uri_string(), options()) ->
    supervisor:startchild_ret().
create_channel_pool(Name, URL, Opts) ->
    % FIXME: return Resource on error
    ...
    case uri_string:parse(URL) of
        #{scheme := Scheme, host := Host, port := Port} ->
            ...
            supervisor:start_child(grpc_sup, Spec);
        {error, Reason, _} ->
            {error, Reason}
    end.
...
ensure_pool_worker(Name, WorkName, Slot) ->
    try
        gproc_pool:add_worker(Name, WorkName, Slot)
    catch
        error:exists -> ok
    end.
